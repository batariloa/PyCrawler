...
async def look_for_links(search_phrase):
    ...
    try:
        search_results = await look_at_links_from_list(5, lista_linkova)
...
async def look_at_links_from_list(number_of_links, list_of_links):
    ...
    results = SearchResults(links_searched, links_found)
    return results
...
async def main():
    results = await look_for_links("ketchup")
